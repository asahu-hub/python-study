...
def remove_all(L, x):
    ...
    return [item for item in L.copy() if item!=x]
...
def decompress_vector(d, n=None):
    ...
    for index,value in zip(d['inds'], d['vals']):
        if index in fullDict:
            value += fullDict[index]
        fullDict[index]=value
    fullDict = dict(sorted(fullDict.items(), key=lambda item : item[0]))
    
    for i in range(n):
        if i not in fullDict:
            fullDict[i]=0.0
    fullDict = dict(sorted(fullDict.items(), key=lambda item : item[0]))
    print("Output Values: ",fullDict.values())
