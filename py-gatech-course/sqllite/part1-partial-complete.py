...
query = ''' SELECT LOWER(ComplaintType) as type, COUNT(*) AS freq
FROM data
GROUP BY type
ORDER BY freq DESC'''
...
query = '''
  SELECT LOWER(ComplaintType) as complaint_type, City AS city_name, COUNT(*) AS complaint_count
    FROM data
    WHERE city_name <> 'None'
        AND city_name IN ("BROOKLYN", "NEW YORK", "BRONX", "STATEN ISLAND", "Jamaica", "Flushing", "ASTORIA")
    GROUP BY city_name COLLATE NOCASE, complaint_type
